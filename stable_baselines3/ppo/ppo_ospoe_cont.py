...
class OSPOEContinuous(OnPolicyAlgorithm):
    ...
    def compute_int_rewards(self, obs : th.Tensor, action : th.Tensor) -> th.Tensor:
        # given next_obs as shape (B, d), return bonus (B, 1)
        if len(obs.shape) > 2:
            obs = obs.view(self.observation_shape)
        
        if self.discrete_action:
            action = action.float().view(-1, 1)
        cat = th.cat([obs, action], dim=1)
        return (self.rnd_predictor(cat) - self.rnd_target(cat)).pow(2).sum(1) / 2
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                int_values = self.int_value_head(obs_tensor)
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data, int_rollout_data in zip(self.rollout_buffer.get(self.batch_size), self.int_rollout_buffer.get(self.batch_size)):
                ...
                if self.discrete_action:
                    actions = int_rollout_data.actions.float().view(-1, 1)
